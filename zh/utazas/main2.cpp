...
int B_feladat (int &N, ut utazas[MAXN])
{
    int legdragabb = 0;
    for(int i = N-1; i >= 0; i--)
    {
    ...
    for(int i = 0; i < N; i++)
    {
        if(utazas[i].tavolsag<1000 && utazas[i].ar >= utazas[legdragabb].ar)
        {
            legdragabb=i;
...
int C_feladat(int &N, ut utazas[MAXN])
{
    int hanyfele = 0;
    bool vane=false;
    for(int i = 0; i < N; i++)
...
void D_feladat(int &N, ut utazas[MAXN])
{
    int szamuk[MAXN];
    int szamdb = 0;
    for(int i = 0; i < N; i++)
